    ...
    public void actionPerformed(ActionEvent e) {
        if (e.getSource().equals(btn_search)) {
            if (!text_search.getText().isEmpty()) {
                text_output.setText("");
                SimpleEntityQuery<SimpleSceneObject> fm = c.objects.populate().filter((n) -> (n.getActions() != null) && n.getActions().length > 0);
                for (SimpleSceneObject sso : fm) {
                    if (sso != null) {
                        if (sso.getActions()[0] != null) {
                            if (sso.getActions()[0].contains(text_search.getText())) {
                                try {
